...
await conn.reply(m.chat, `@${tagUser.split("@")[0]} ESTÁ JUGANDO A LA SOPA DE PALABRAS 🔠 EN ESTE MOMENTO`, m, { mentions: [tagUser] })
return
}
if (intentos === 0) {
intentos = 3  
...
if (intentos !== 0) {
await conn.reply( m.chat, `@${m.sender.split("@")[0]} SE ACABÓ EL TIEMPO!! 😧\n\n LA PALABRA"${sopaPalabra}"ESTABA EN LA DIRECCIÓN${cambioLetra}DESDE LA FILA ${fila}Y COLUMNA ${columna}`, m, { mentions: [m.sender] })
...
await m.reply(`\`\`\`🎊 USTED HA GANADO ${diamante} ${rpgshop.emoticon('limit')}!!\`\`\`\n\n ¡¡CORRECTO!! LA PALABRA "${sopaPalabra}"ESTABA EN LA DIRECCIÓN ${cambioLetra}DESDE LA FILA ${fila}Y COLUMNA${columna}`)
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
}else{
if (intentos === 1) {
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
await m.reply(`🫡 ¡¡AGOTAS TU INTENTO!! LA PALABRA"${sopaPalabra}"ESTABA EN LA DIRECCIÓN${cambioLetra}DESDE LA FILA ${fila}Y COLUMNA${columna}`)
return  
} else {
intentos -= 1
await m.reply(`😮‍💨 INCORRECTO. TE QUEDAN${intentos} INTENTOS!!${intentos === 1 ? '' : `\n PARA ENCONTRAR LA PALABRA: \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`💡 CLAVE!!\`\`\`\n LA PALABRA${sopaPalabra}ENCONTRADO EN LA DIRECCIÓN"${cambioLetra}"\n\n` : ''}${sopaNube}`)
}}
}}
